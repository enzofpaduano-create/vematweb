...
import {
  listDocuments, DOC_LABEL, DOC_LABEL_SHORT, formatMoney, groupIntoFolders,
  type PdrDocument, type PdrDocType, type PdrFolder,
...
export default function PdrDocuments() {
  ...
  useEffect(() => {
    ...
    listDocuments()
      .then((d) => {
        if (cancelled) return;
        setDocs(d);
        setOpenFolders(new Set(groupIntoFolders(d).map((f) => f.id)));
      })
  ...
  const folders = useMemo(() => {
    const allFolders = groupIntoFolders(docs ?? []);
    const q = query.trim().toLowerCase();
    return allFolders
      .map((folder): PdrFolder | null => {
        let list = folder.docs;
        if (filter !== "all") list = list.filter((d) => d.type === filter);
        if (q) {
          const folderMatch =
            folder.client.toLowerCase().includes(q) ||
            (folder.root.reference ?? "").toLowerCase().includes(q);
          if (!folderMatch) {
            list = list.filter((d) =>
      ...
      .filter((f): f is PdrFolder => f !== null);
  }, [docs, filter, query]);
      ...
      <PdrLayout>
        <div className="p-6 lg:p-10 max-w-[1400px] mx-auto">
          ...
          <div className="relative mb-6 max-w-md">
            ...
            {query && (
              <button onClick={() => setQuery("")} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-md text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100" aria-label="Clear"><X className="h-4 w-4" /></button>
            )}
          </div>
