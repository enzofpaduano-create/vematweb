      ...
      <AdminLayout>
        <div className="p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-black text-zinc-900">{t("portal.clients.title")}</h1>
              <p className="text-sm text-zinc-500 mt-1">{clients.length} {clients.length !== 1 ? t("portal.clients.companies") : t("portal.clients.company")}</p>
            </div>
            {!loading && (
              <div className="flex items-center gap-1.5 bg-white border border-zinc-100 rounded-xl px-4 py-2.5">
                <TrendingUp className="w-4 h-4 text-emerald-600" />
                <span className="text-sm font-black text-zinc-900">{totalRevenue.toLocaleString(lang === "fr" ? "fr-FR" : "en-GB")} MAD</span>
                <span className="text-xs text-zinc-400">{t("portal.clients.totalBilled")}</span>
              </div>
            )}
          </div>
          ...
          <div className="relative mb-5">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("portal.clients.searchPlaceholder")}
          ...
          {loading ? (
            <div className="bg-white rounded-xl border border-zinc-100 py-16 text-center text-zinc-400">{t("portal.common.loading")}</div>
          ) : (
            <div className="bg-white rounded-xl border border-zinc-100 overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-100 bg-zinc-50">
                    <th className="text-left px-5 py-3 text-xs font-black uppercase tracking-wider text-zinc-400">{t("portal.clients.companyCol")}</th>
                    <th className="text-left px-5 py-3 text-xs font-black uppercase tracking-wider text-zinc-400">{t("portal.clients.contactCol")}</th>
                    <th className="text-center px-4 py-3 text-xs font-black uppercase tracking-wider text-zinc-400">{t("portal.clients.ordersCol")}</th>
                    <th className="text-center px-4 py-3 text-xs font-black uppercase tracking-wider text-zinc-400">{t("portal.clients.repairsCol")}</th>
                    <th className="text-right px-5 py-3 text-xs font-black uppercase tracking-wider text-zinc-400">{t("portal.clients.revenueCol")}</th>
                    ...
                    <tr key={c.id} className="hover:bg-zinc-50 transition-colors group">
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          {c.activeOrdersCount > 0 && (
                            <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full">{c.activeOrdersCount} {t("portal.clients.active")}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-4 py-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          {c.activeRepairsCount > 0 && (
                            <span className="text-[10px] font-bold text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded-full">{c.activeRepairsCount} {t("portal.clients.ongoing")}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-5 py-4 text-right">
                        <span className="font-black text-zinc-900">
                          {c.ordersTotal > 0 ? `${c.ordersTotal.toLocaleString(lang === "fr" ? "fr-FR" : "en-GB")} MAD` : "—"}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <Link href={`/espace-manager/clients/${c.id}`}
                          ...
                          {t("portal.clients.see")} <ChevronRight className="w-3.5 h-3.5" />
                        </Link>
                      </td>
                    </tr>
                  ))}
