...
export default function DGCommandes() {
  ...
  const PERIOD_LABELS: Record<Period, string> = {
    semaine: t("portal.orders.periodLabels.semaine"),
    mois: t("portal.orders.periodLabels.mois"),
    annee: t("portal.orders.periodLabels.annee"),
  };
      ...
      <DGLayout>
        <div className="p-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-black text-zinc-900">{t("portal.orders.titleDG")}</h1>
              <p className="text-sm text-zinc-500 mt-1">{t("portal.orders.subtitleDG")} · {filtered.length} {t("portal.orders.statsOrders").toLowerCase()}</p>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            {[
              { icon: ShoppingCart, label: `${t("portal.orders.statsOrders")} · ${PERIOD_LABELS[period]}`, value: loading ? "—" : String(periodOrders.length), color: "text-blue-600 bg-blue-50" },
              { icon: TrendingUp, label: t("portal.orders.statsValue"), value: loading ? "—" : totalValue.toLocaleString(lang === "fr" ? "fr-FR" : "en-GB"), color: "text-emerald-600 bg-emerald-50" },
              { icon: CheckCircle2, label: t("portal.orders.statsValidated"), value: loading ? "—" : String(validatedCount), color: "text-purple-600 bg-purple-50" },
              { icon: Clock, label: t("portal.orders.statsPending"), value: loading ? "—" : String(pendingValidation), color: "text-amber-600 bg-amber-50" },
          ...
          <div className="grid grid-cols-3 gap-6 mb-6">
            ...
            <div className="col-span-2 bg-white rounded-xl border border-zinc-100 p-6">
              <h2 className="text-xs font-black uppercase tracking-wider text-zinc-500 mb-5">
                {t("portal.orders.breakdown")} · {PERIOD_LABELS[period]}
              </h2>
              {loading ? (
                <div className="text-sm text-zinc-400 py-4">{t("portal.common.loading")}</div>
              ) : periodOrders.length === 0 ? (
                <div className="text-sm text-zinc-400 py-4">{t("portal.orders.noOrdersThisPeriod")}</div>
              ) : (
            ...
            <div className="bg-white rounded-xl border border-zinc-100 p-6 flex flex-col justify-between">
              <div>
                <h2 className="text-xs font-black uppercase tracking-wider text-zinc-500 mb-4">{t("portal.orders.avgBasket")}</h2>
                <p className="text-3xl font-black text-zinc-900">
                  {loading ? "—" : avgValue > 0 ? avgValue.toLocaleString(lang === "fr" ? "fr-FR" : "en-GB", { maximumFractionDigits: 0 }) : "—"}
                </p>
                <p className="text-sm text-zinc-400 mt-1">{t("portal.orders.madPerOrder")}</p>
              </div>
              <div className="mt-6 space-y-2 border-t border-zinc-100 pt-4">
                <div className="flex justify-between text-xs">
                  <span className="text-zinc-500">{t("portal.orders.withQuote")}</span>
                  <span className="font-bold text-zinc-900">{periodOrders.filter((o) => o.quote_amount).length}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-zinc-500">{t("portal.orders.withoutQuote")}</span>
                  <span className="font-bold text-zinc-900">{periodOrders.filter((o) => !o.quote_amount).length}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-zinc-500">{t("portal.orders.cancelled")}</span>
          ...
          <div className="bg-white rounded-xl border border-zinc-100 overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-4 border-b border-zinc-100">
              <div className="relative flex-1 max-w-xs">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)}
                  placeholder={t("portal.orders.searchPlaceholderDG")}
            ...
            {loading ? (
              <div className="py-16 text-center text-sm text-zinc-400">{t("portal.common.loading")}</div>
            ) : filtered.length === 0 ? (
              <div className="py-16 text-center text-sm text-zinc-400">{t("portal.orders.noOrders")}</div>
            ) : (
                  ...
                  {filtered.map((o) => (
                    <tr key={o.id} className="hover:bg-zinc-50 transition-colors cursor-pointer">
                      ...
                      <td className="px-5 py-3.5 text-zinc-500 text-xs">
                        {new Date(o.created_at).toLocaleDateString(lang === "fr" ? "fr-FR" : "en-GB", { day: "numeric", month: "short", year: "numeric" })}
                      </td>
                      <td className="px-5 py-3.5 font-semibold text-zinc-700">
                        {o.quote_amount ? `${o.quote_amount.toLocaleString(lang === "fr" ? "fr-FR" : "en-GB")} MAD` : "—"}
                      </td>
                      <td className="px-5 py-3.5">
