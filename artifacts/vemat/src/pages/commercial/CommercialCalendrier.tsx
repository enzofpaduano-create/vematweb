    ...
    <CommercialLayout>
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-black text-white">Planning commercial</h1>
            <p className="text-zinc-500 text-sm mt-0.5">Visites clients, réunions et appels</p>
          </div>
              ...
              <div key={i} className={`rounded-xl border overflow-hidden transition-all ${isToday ? "border-sky-500/40 bg-sky-500/5" : isPast ? "border-zinc-800 opacity-60" : "border-zinc-800"}`}>
                <div className={`px-5 py-3 flex items-center justify-between border-b ${isToday ? "border-sky-500/20 bg-sky-500/10" : "border-zinc-800 bg-zinc-900"}`}>
                  <div className="flex items-center gap-3">
                    ...
                    {isToday && <span className="text-[10px] font-black uppercase tracking-wider text-sky-400 bg-sky-500/20 px-2 py-0.5 rounded-full">Aujourd'hui</span>}
                  </div>
                  <div className="flex items-center gap-3">
                    {dayEvs.length > 0 && <span className="text-xs font-bold text-zinc-500">{dayEvs.length} événement{dayEvs.length > 1 ? "s" : ""}</span>}
                ...
                {loading ? (
                  ...
                  <div className="px-5 py-4 text-center text-zinc-700 text-sm">Aucun événement</div>
                ) : (
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800">
              <h3 className="font-black text-white">{editEvent ? "Modifier l'événement" : "Nouvel événement"}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">Titre *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">Type</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">Heure début</label>
                  <input type="time" value={form.start_time} onChange={(e) => setForm((f) => ({ ...f, start_time: e.target.value }))}
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-sky-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">Heure fin</label>
