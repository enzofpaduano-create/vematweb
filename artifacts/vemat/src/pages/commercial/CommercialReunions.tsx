...
function getOutcomes(t: (k: string) => string) {
  return [
    { value: "positif",       label: t("portal.commercial_page.outcomeLabels.positif"),       color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" },
    { value: "neutre",        label: t("portal.commercial_page.outcomeLabels.neutre"),        color: "text-blue-400 bg-blue-500/10 border-blue-500/30" },
    { value: "negatif",       label: t("portal.commercial_page.outcomeLabels.negatif"),       color: "text-red-400 bg-red-500/10 border-red-500/30" },
    { value: "a_recontacter", label: t("portal.commercial_page.outcomeLabels.a_recontacter"), color: "text-orange-400 bg-orange-500/10 border-orange-500/30" },
  ];
}
...
export default function CommercialReunions() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    let error;
    if (editReport) {
      ({ error } = await supabaseCommercial.from("commercial_meeting_reports").update(payload).eq("id", editReport.id));
    } else {
      ({ error } = await supabaseCommercial.from("commercial_meeting_reports").insert(payload));
    }
    setSaving(false);
  ...
  const handleDelete = async (r: CommercialMeetingReport) => {
    if (!confirm(t("portal.commercial_page.confirmDeleteReport"))) return;
    ...
    <CommercialLayout>
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-black text-white">{t("portal.commercial_page.reportsTitle")}</h1>
            <p className="text-zinc-500 text-sm mt-0.5">{t("portal.commercial_page.reportsSubtitle")}</p>
          </div>
          <button onClick={openCreate}
            ...
            <Plus className="w-4 h-4" /> {t("portal.commercial_page.newReport")}
          </button>
        </div>

        {/* Outcome filter */}
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          {[{ value: "all", label: t("portal.common.all") }, ...OUTCOMES].map((o) => (
        ...
        {loading ? (
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl py-16 text-center">
            ...
            <p className="text-zinc-500 text-sm">{t("portal.commercial_page.noReports")}</p>
          </div>
        ) : (
                ...
                <div key={r.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-colors">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">
                        {new Date(r.date + "T00:00:00").toLocaleDateString(lang === "fr" ? "fr-FR" : "en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
                      </p>
                    </div>
                  ...
                  {r.next_step && (
                    <div className="flex items-start gap-2 bg-sky-500/10 border border-sky-500/20 rounded-lg px-3 py-2">
                      ...
                      <p className="text-xs text-sky-300"><span className="font-bold">{t("portal.commercial_page.nextStep")} :</span> {r.next_step}</p>
                    </div>
                  )}
                  {/* Validation DG */}
                  <div className="flex items-center gap-2 mt-3">
                    {r.validated_by_dg ? (
                      <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full">
                        <Check className="w-3 h-3" /> {t("portal.commercial_page.validatedByDG")}
                      </span>
                    ) : (
                      <span className="text-[10px] font-bold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-full">
                        {t("portal.commercial_page.awaitingValidation")}
                      </span>
                    )}
                  </div>
                  {r.validated_by_dg && r.dg_comment && (
                    <div className="mt-2 flex items-start gap-2 bg-sky-500/10 border border-sky-500/20 rounded-lg px-3 py-2">
                      <AlertCircle className="w-3.5 h-3.5 text-sky-400 shrink-0 mt-0.5" />
                      <p className="text-xs text-sky-300"><span className="font-bold">{t("portal.commercial_page.dgNote")} :</span> {r.dg_comment}</p>
                    </div>
                  )}
                </div>
              );
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800 sticky top-0 bg-zinc-900">
              <h3 className="font-black text-white">{editReport ? t("portal.commercial_page.editReport") : t("portal.commercial_page.newReport")}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.client_name} onChange={(e) => setForm((f) => ({ ...f, client_name: e.target.value }))} required
                    placeholder={t("portal.commercial_page.formClientPlaceholder")} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3.5 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-sky-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">{t("portal.common.date")} *</label>
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">{t("portal.commercial_page.formSummary")}</label>
                <textarea value={form.summary} onChange={(e) => setForm((f) => ({ ...f, summary: e.target.value }))} rows={4}
                  placeholder={t("portal.commercial_page.formSummaryPlaceholder")}
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">{t("portal.commercial_page.formOutcome")}</label>
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wide mb-1.5">{t("portal.commercial_page.formNextStep")}</label>
                <input value={form.next_step} onChange={(e) => setForm((f) => ({ ...f, next_step: e.target.value }))}
                  placeholder={t("portal.commercial_page.formNextStepPlaceholder")}
