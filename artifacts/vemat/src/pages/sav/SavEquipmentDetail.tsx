...
export default function SavEquipmentDetail() {
  const [, params] = useRoute<{ id: string }>("/espace-sav/equipment/:id");
  ...
  async function reload() {
    if (!id) return;
    try {
      const eq = await getEquipment(supabaseSav, id);
      if (!eq) { setError("Equipment not found."); setLoading(false); return; }
      setEquipment(eq);
      const [cl, hist] = await Promise.all([
        getClient(supabaseSav, eq.client_id),
        getEquipmentHistory(supabaseSav, id),
      ]);
      setClient(cl);
      setHistory(hist);
      setLoading(false);
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      await updateEquipment(supabaseSav, equipment.id, {
        serial_number: equipment.serial_number, brand: equipment.brand,
        model: equipment.model, machine_type: equipment.machine_type,
        year_manufacture: equipment.year_manufacture, purchase_date: equipment.purchase_date,
        hour_meter: equipment.hour_meter, status: equipment.status, notes: equipment.notes,
      });
      setSaved(true);
  ...
  async function handleDelete() {
    ...
    try {
      await deleteEquipment(supabaseSav, equipment.id);
      if (client) navigate(`/espace-sav/client/${client.id}`);
      else navigate("/espace-sav/equipments");
    } catch (e) { setError((e as Error).message); }
  }
    ...
    <SavGuard>
      <SavLayout>
        <div className="p-6 lg:p-10 max-w-4xl mx-auto">
          <button onClick={() => navigate("/espace-sav/equipments")} className="text-xs text-zinc-500 hover:text-emerald-600 inline-flex items-center gap-1 mb-4 transition-colors">
          ...
          {equipment && (
            <>
              <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] font-black text-emerald-600 mb-1">Equipment</p>
                  <h1 className="text-3xl font-black text-zinc-950">
                    <Wrench className="w-6 h-6 inline mr-2 text-emerald-500" />
                    {equipment.brand || "—"} {equipment.model || ""}
                  </h1>
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    <span className={`text-[10px] font-black uppercase tracking-wide px-2 py-1 rounded ${EQUIPMENT_STATUS_COLOR[equipment.status]}`}>{EQUIPMENT_STATUS_LABEL[equipment.status]}</span>
                    {equipment.serial_number && <span className="font-mono text-xs text-zinc-500">S/N: {equipment.serial_number}</span>}
                    {client && <Link href={`/espace-sav/client/${client.id}`} className="text-emerald-600 hover:underline text-xs font-semibold">{client.name} ↗</Link>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select className={ic} value={equipment.status} onChange={(e) => setEquipment({ ...equipment, status: e.target.value as EquipmentStatus })}>
                      {(Object.keys(EQUIPMENT_STATUS_LABEL) as EquipmentStatus[]).map((k) => <option key={k} value={k}>{EQUIPMENT_STATUS_LABEL[k]}</option>)}
                    </select>
                  </div>
              ...
              <section className="bg-white rounded-2xl border border-zinc-200 p-6 mb-5">
                <h2 className="font-black text-zinc-950 mb-4 flex items-center gap-2"><History className="w-4 h-4 text-emerald-600" /> 360° history ({history.length})</h2>
                <p className="text-xs text-zinc-500 mb-4">All PDR and SAV documents that reference this specific machine.</p>
                {history.length === 0 && <p className="text-sm text-zinc-500 text-center py-4">No history yet.</p>}
                {history.length > 0 && (
