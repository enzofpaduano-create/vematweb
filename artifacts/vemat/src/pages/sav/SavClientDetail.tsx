...
import { listSavDocuments, DOC_LABEL_SHORT, formatMoney, type SavDocument } from "@/lib/savDocuments";

export default function SavClientDetail() {
  const [, params] = useRoute<{ id: string }>("/espace-sav/client/:id");
  const [, navigate] = useLocation();
  const id = params?.id;

  const [client, setClient] = useState<Client | null>(null);
  const [equipments, setEquipments] = useState<ClientEquipment[]>([]);
  const [docs, setDocs] = useState<SavDocument[]>([]);
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      await updateClient(supabaseSav, client.id, {
        name: client.name,
  ...
  async function handleDelete() {
    ...
    try {
      await deleteClient(supabaseSav, client.id);
      navigate("/espace-sav/clients");
    } catch (e) { setError((e as Error).message); }
  }
  ...
  async function handleCreateEquipment() {
    ...
    try {
      const created = await createEquipment(supabaseSav, {
        client_id: client.id,
    ...
    <SavGuard>
      <SavLayout>
        <div className="p-6 lg:p-10 max-w-4xl mx-auto">
          <Link href="/espace-sav/clients" className="text-xs text-zinc-500 hover:text-emerald-600 inline-flex items-center gap-1 mb-4 transition-colors">
          ...
          {client && (
            <>
              <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] font-black text-emerald-600 mb-1">Client</p>
                  <h1 className="text-3xl font-black text-zinc-950">{client.name}</h1>
                  <div className="flex items-center gap-2 mt-2">
                    <span className={`text-[10px] font-black uppercase tracking-wide px-2 py-1 rounded ${CLIENT_STATUS_COLOR[client.status]}`}>{CLIENT_STATUS_LABEL[client.status]}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={handleSave} disabled={saving} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm px-4 py-2.5 rounded-xl transition-colors disabled:opacity-60">
                ...
                <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center gap-3 mb-6">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-red-700">
                      New billable service offers for this client will be blocked. Override requires N+1 approval.
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select className={ic} value={client.payment_terms} onChange={(e) => setClient({ ...client, payment_terms: e.target.value as PaymentTerms })}>
                      {(Object.keys(PAYMENT_TERMS_LABEL) as PaymentTerms[]).map((k) => <option key={k} value={k}>{PAYMENT_TERMS_LABEL[k]}</option>)}
                    </select>
                  </div>
                  <div><label className={lbl}>Credit limit</label><input className={ic} type="number" min="0" step="100" value={client.credit_limit} onChange={(e) => setClient({ ...client, credit_limit: Number(e.target.value) })} /></div>
                  <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select className={ic} value={client.status} onChange={(e) => setClient({ ...client, status: e.target.value as ClientStatus })}>
                      {(Object.keys(CLIENT_STATUS_LABEL) as ClientStatus[]).map((k) => <option key={k} value={k}>{CLIENT_STATUS_LABEL[k]}</option>)}
                    </select>
                  ...
                  <div><label className={lbl}>Reason (if blocked/dispute)</label><input className={ic} value={client.status_reason ?? ""} onChange={(e) => setClient({ ...client, status_reason: e.target.value || null })} /></div>
                </div>
                <div className="mt-4"><label className={lbl}>Internal notes</label><textarea className={`${ic} min-h-[60px]`} value={client.notes ?? ""} onChange={(e) => setClient({ ...client, notes: e.target.value || null })} /></div>
              </section>

              <section className="bg-white rounded-2xl border border-zinc-200 p-6 mb-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-black text-zinc-950 flex items-center gap-2"><Wrench className="w-4 h-4 text-emerald-600" /> Equipment park ({equipments.length})</h2>
                  <button onClick={() => setShowNewEquip(true)} className="inline-flex items-center gap-1.5 text-sm font-bold text-emerald-600 hover:text-emerald-500"><Plus className="w-4 h-4" /> Add equipment</button>
                </div>
                {showNewEquip && (
                  <div className="border border-emerald-200 rounded-xl p-3 mb-3 bg-emerald-50/50">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2">
                      <input className={ic} placeholder="Brand" value={newEquip.brand} onChange={(e) => setNewEquip({ ...newEquip, brand: e.target.value })} />
                      <input className={ic} placeholder="Model" value={newEquip.model} onChange={(e) => setNewEquip({ ...newEquip, model: e.target.value })} />
                      ...
                      <input className={ic} placeholder="Type" value={newEquip.machine_type} onChange={(e) => setNewEquip({ ...newEquip, machine_type: e.target.value })} />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={handleCreateEquipment} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-3 py-1.5 rounded-lg">Add</button>
                ...
                {equipments.length === 0 && !showNewEquip && <p className="text-sm text-zinc-500 text-center py-4">No equipment yet for this client.</p>}
                {equipments.length > 0 && (
                  <div className="space-y-2">
                    {equipments.map((eq) => (
                      <Link key={eq.id} href={`/espace-sav/equipment/${eq.id}`}>
                        <div className="border border-zinc-200 hover:border-emerald-300 rounded-xl px-4 py-3 flex items-center justify-between gap-3 cursor-pointer transition-colors">
                          <div className="min-w-0">
                            <p className="font-bold text-zinc-950 text-sm">{eq.brand || "—"} {eq.model || ""}{eq.machine_type && <span className="text-zinc-400 font-normal"> · {eq.machine_type}</span>}</p>
                            <p className="font-mono text-xs text-zinc-500 mt-0.5">S/N: {eq.serial_number || "—"}{eq.hour_meter != null && <span className="text-zinc-400"> · {eq.hour_meter} h</span>}</p>
                          </div>
                          <span className={`text-[10px] font-black uppercase tracking-wide px-2 py-1 rounded shrink-0 ${EQUIPMENT_STATUS_COLOR[eq.status]}`}>{EQUIPMENT_STATUS_LABEL[eq.status]}</span>
                        </div>
                      </Link>
                    ))}
                  </div>
              ...
              <section className="bg-white rounded-2xl border border-zinc-200 p-6 mb-5">
                <h2 className="font-black text-zinc-950 mb-4 flex items-center gap-2"><FileText className="w-4 h-4 text-emerald-600" /> Documents ({clientDocs.length})</h2>
                {clientDocs.length === 0 && <p className="text-sm text-zinc-500 text-center py-4">No SAV documents linked to this client yet.</p>}
                {clientDocs.length > 0 && (
